...
void Engine::create(int screenWidth, int screenHeight, const std::string &name, bool verticalSync, sf::Color background,
    ...
    while (screen->isOpen()) {
        ...
        if (_updateWorld) {
            ...
            int substeps = 0;
            while (physicsAccumulator >= Consts::PHYSICS_FIXED_DT && substeps < Consts::PHYSICS_MAX_SUBSTEPS) {
                world->stepPhysics(Consts::PHYSICS_FIXED_DT);
                physicsAccumulator -= Consts::PHYSICS_FIXED_DT;
                substeps++;
            }
            if (substeps >= Consts::PHYSICS_MAX_SUBSTEPS) {
