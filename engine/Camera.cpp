...
std::vector<std::shared_ptr<Triangle>> Camera::project(std::shared_ptr<Mesh> mesh) {
    ...
    size_t beforeCount = _triangles.size();

    for (auto &t : mesh->triangles()) {

        Triangle MTriangle = t * M;

        Vec3D center = (Vec3D(MTriangle[0]) + Vec3D(MTriangle[1]) + Vec3D(MTriangle[2])) / 3.0;
        double dot_cull = MTriangle.norm().dot((center - position()).normalized());

        if (dot_cull > 0) {
            continue;
        }

        double dot_light = std::abs(MTriangle.norm().dot(lookAt().normalized()));
        ...
        for (auto &clipped : clippedTriangles) {
            ...
            sf::Color ambientColor = sf::Color(static_cast<sf::Uint8>(color.r * (0.3 * dot_light + 0.7)),
                                               static_cast<sf::Uint8>(color.g * (0.3 * dot_light + 0.7)),
                                               static_cast<sf::Uint8>(color.b * (0.3 * dot_light + 0.7)),
    ...
    _bodyTrisCount[mesh->name().str()] = _triangles.size() - beforeCount;
...
std::vector<std::shared_ptr<Triangle>> Camera::sorted() {
    ...
    std::sort(_triangles.begin(), _triangles.end(), [](const std::shared_ptr<Triangle> &t1, const std::shared_ptr<Triangle> &t2) {
