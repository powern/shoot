...
struct AABB final {
    ...
    [[nodiscard]] Vec3D center() const { return (min + max) * 0.5; }
    [[nodiscard]] Vec3D extents() const { return (max - min) * 0.5; }
...
public:
    ...
    void solveCollision(const CollisionPoint &collision, std::shared_ptr<RigidBody> other);
    ...
    [[nodiscard]] Vec3D angularAcceleration() const { return _angularAcceleration; }
    void setAngularVelocity(const Vec3D &av);
    void addAngularVelocity(const Vec3D &av);
    void setAngularAcceleration(const Vec3D &aa);

    // Physical properties
    [[nodiscard]] double mass() const { return _mass; }
    [[nodiscard]] double invMass() const { return _invMass; }
    [[nodiscard]] double inertia() const { return _inertia; }
    [[nodiscard]] double invInertia() const { return _invInertia; }
    [[nodiscard]] double restitution() const { return _restitution; }
    [[nodiscard]] double friction() const { return _friction; }
    [[nodiscard]] bool isStatic() const { return _isStatic; }

    void setMass(double m);
    void setRestitution(double r) { _restitution = r; }
    void setFriction(double f) { _friction = f; }
    ...
    void updatePhysicsState(double dt);
